...
export const SiteDataProvider = ({ children }) => {
  ...
  const footerServices = Array.isArray(owner.footerServices)
    ? owner.footerServices
    : [];
  const faqs = Array.isArray(owner.faqs) ? owner.faqs : [];
  ...
  const servicesOps = {
    add: useCallback(async (item) => {
      const d = await apiCreateService(item);
      setServices((p) => [...p, d]);
      return d;
    }, []),
    update: useCallback(async (id, b) => {
      const d = await apiUpdateService(id, b);
      setServices((p) => p.map((s) => (s._id === id ? d : s)));
      return d;
    }, []),
  ...
  const portfolioOps = {
    add: useCallback(async (item) => {
      const d = await apiCreatePortfolio(item);
      setPortfolio((p) => [...p, d]);
      return d;
    }, []),
    update: useCallback(async (id, b) => {
      const d = await apiUpdatePortfolio(id, b);
      setPortfolio((p) => p.map((i) => (i._id === id ? d : i)));
      return d;
    }, []),
    remove: useCallback(async (id) => {
  ...
  const testimonialsOps = {
    add: useCallback(async (item) => {
      const d = await apiCreateTestimonial(item);
      setTestimonials((p) => [...p, d]);
      return d;
    }, []),
    update: useCallback(async (id, b) => {
      const d = await apiUpdateTestimonial(id, b);
      setTestimonials((p) => p.map((t) => (t._id === id ? d : t)));
      return d;
    }, []),
    remove: useCallback(async (id) => {
  ...
  const pricingOps = {
    add: useCallback(async (item) => {
      const d = await apiCreatePricing(item);
      setPricing((p) => [...p, d]);
      return d;
    }, []),
    update: useCallback(async (id, b) => {
      const d = await apiUpdatePricing(id, b);
      setPricing((p) => p.map((i) => (i._id === id ? d : i)));
      return d;
    }, []),
    remove: useCallback(async (id) => {
  ...
  const updateFaqs = useCallback(
    async (newFaqs) => {
      const doc = await apiUpdateAbout({ ...owner, faqs: newFaqs });
      if (doc && typeof doc === "object") setOwner(doc);
      return doc;
    },
    [owner],
