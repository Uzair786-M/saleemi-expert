      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "1rem" }}>
            <Link to="/contact" style={{ padding: "clamp(12px, 1.2vw, 20px) clamp(24px, 2.5vw, 48px)", fontWeight: 600, borderRadius: "0.75rem", backgroundColor: "#06b6d4", color: "#ffffff", textDecoration: "none", transition: "background-color 0.2s", fontSize: "clamp(0.875rem, 1.2vw, 1.25rem)" }}
              onMouseEnter={e => e.currentTarget.style.backgroundColor = "#22d3ee"}
              onMouseLeave={e => e.currentTarget.style.backgroundColor = "#06b6d4"}
            >Get Started</Link>

            <Link to="/portfolio" style={{ padding: "clamp(12px, 1.2vw, 20px) clamp(24px, 2.5vw, 48px)", fontWeight: 600, borderRadius: "0.75rem", border: "1px solid rgba(255,255,255,0.2)", color: "#ffffff", textDecoration: "none", backgroundColor: "transparent", transition: "border-color 0.2s, color 0.2s", fontSize: "clamp(0.875rem, 1.2vw, 1.25rem)" }}
          ...
          {/* ── Animated Stats ── */}
          <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(displayStats.length, 4)}, 1fr)`, gap: "1.5rem", marginTop: "3.5rem" }}>
            {displayStats.map((stat, i) => (
              <div key={i}>
                <h2 style={{ color: "#22d3ee", fontSize: "clamp(1.5rem, 2.5vw, 3rem)", fontWeight: 700, lineHeight: 1 }}>
                  <AnimatedCounter
                ...
                <p style={{ color: "#9ca3af", fontSize: "clamp(0.75rem, 1vw, 1.1rem)", marginTop: "4px" }}>{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "center", position: "relative" }}>
          <div style={{ position: "absolute", width: "40%", height: "40%", borderRadius: "9999px", backgroundColor: "rgba(6,182,212,0.15)", filter: "blur(80px)", zIndex: 0 }} />
          <div style={{ position: "relative", zIndex: 1, width: "100%", maxWidth: "520px", padding: "clamp(1.5rem, 2vw, 3rem)", borderRadius: "1.5rem", backgroundColor: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", backdropFilter: "blur(20px)", boxShadow: "0 25px 50px rgba(0,0,0,0.4)" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
              {HERO_CARDS.map(card => (
                <div key={card.label} style={{ padding: "clamp(1rem, 1.5vw, 1.75rem)", borderRadius: "1rem", backgroundColor: "#0d1224", border: "1px solid rgba(255,255,255,0.1)" }}>
